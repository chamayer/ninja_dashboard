...
def _software_rollup_rows(*, client_id=None) -> list[dict]:
    ...
    """
    # Reads the exposure view directly. This aggregated the whole set, which is
    # why migration 083 pre-computed it -- but once `whitelist_suggestion`
    # stopped fanning out (084), 91% of the rows went away and the aggregate
    # became unnecessary. Live means approvals apply immediately here too,
    # with no lag to explain.
