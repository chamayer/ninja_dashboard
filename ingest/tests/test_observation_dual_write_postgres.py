"""Opt-in PostgreSQL coverage for ADR-0009 dual writes."""
...
def test_current_history_and_run_dual_write(postgres_connection) -> None:
    ...
    with postgres_connection.transaction(), postgres_connection.cursor() as cursor:
        ...
        assert written == 1
        complete_run(cursor, run_id, written, is_complete_snapshot=True)
        ...
        cursor.execute(
            ...
            SELECT source_instance_id, run_started_at = snapshot_at,
                   status, is_complete_snapshot
              FROM operations.observation_snapshot_runs
             WHERE run_id = %s
        """,
            (run_id,),
        )
        assert cursor.fetchone() == (instance_id, True, "complete", True)
