...
## [0.126.16] — 2026-09-23 — Jobs status accuracy

### Fixed

- Made the Jobs catalog use durable queue history for registered work, with
  legacy run-log history retained as a fallback.
- Show queue-backed jobs as queued, running, cancelled, or completed instead
  of incorrectly reporting completed work as "Never run."
- Mark disabled legacy Agent compliance bridge jobs as disabled and remove
  their no-op run controls.
...
- Added a No platform records option to Computers → Platforms, showing
  Computers without a current management or security platform record.
...
- Matching top-row and table-column filter checkboxes now stay synchronized,
  so clearing archived Hudu records clears the submitted filter.

## [0.122.12] — 2026-09-07 — Archived Hudu inventory filter reliability

### Fixed

- Computers no longer times out when filtering archived Hudu records to those
  without links.
