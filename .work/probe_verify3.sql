...
-- Full rewrite, with every FILTER the view uses.
WITH scoped_devices AS (
    SELECT v.device_id, v.client_id, v.canonical_hostname, v.device_role, v.os_group,
           v.last_contact_at, v.needs_reboot, v.effective_patching_scope
    ...
    WHERE dl.tenant_id = 1 AND LOWER(s.name) = 'ninja' AND dl.external_id ~ '^\d+$'
), patch_signal AS (
    SELECT nl.device_id, BOOL_OR(COALESCE(dps.ever_installed, FALSE)) AS ever_installed
    FROM ninja_links nl
    JOIN ninja_patches.device_patch_signal dps ON dps.device_id = nl.ninja_device_id
    GROUP BY nl.device_id
), patch_activity AS (
    SELECT nl.device_id, MAX(dpa.last_patch_activity_at) AS last_patch_activity_at
    FROM ninja_links nl
    JOIN ninja_patches.device_patch_activity dpa ON dpa.device_id = nl.ninja_device_id
    GROUP BY nl.device_id
), device_posture AS (
    SELECT sd.*, pa.last_patch_activity_at,
           ...
           pa.last_patch_activity_at >= NOW() - INTERVAL '35 days' AS has_recent_patch_activity
...
SELECT dp.client_id, c.slug, c.display_name,
       ...
       COUNT(*) FILTER (WHERE dp.is_active AND dp.effective_patching_scope='Included' AND dp.has_recent_patch_activity)::int AS recent_activity,
       COUNT(*) FILTER (WHERE dp.is_active AND dp.effective_patching_scope='Included' AND NOT dp.has_recent_patch_activity)::int AS quiet,
       COUNT(*) FILTER (WHERE dp.is_active AND dp.effective_patching_scope='Included' AND NOT dp.ever_installed)::int AS never_patched,
       COUNT(*) FILTER (WHERE dp.is_active AND dp.effective_patching_scope='Included' AND dp.needs_reboot)::int AS reboot_pending,
...
ORDER BY is_total DESC, quiet DESC, never_patched DESC, reboot_pending DESC, c.display_name
LIMIT 5;
