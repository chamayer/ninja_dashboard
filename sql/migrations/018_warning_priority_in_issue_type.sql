-- =============================================================================
-- 018_warning_priority_in_issue_type.sql
-- Operator feedback: when a device has BOTH warnings (MESSAGE events)
-- AND delayed patches, the troubleshooting signal should report it as
-- "Stalled with warnings" rather than "Stalled with delayed patches".
-- Rationale: delayed patches are normal state (auto-approve window),
-- warnings are operator-actionable signals.
--
-- Single CASE-block insert: "Stalled with warnings" branch added
-- BETWEEN "Stalled with manual approvals" and "Stalled with delayed
-- patches". Same insert in the suggested_action CASE.
--
-- Otherwise identical to migration 017's device_troubleshooting_signal
-- (drop + recreate with the same dependencies).
-- =============================================================================
...
SELECT
    ...
    CASE
        ...
        WHEN dps.last_seen_at < NOW() - INTERVAL '35 days' AND COALESCE(ar.warning_events_30d, 0) > 0 THEN 'Stalled with warnings'
        ...
        WHEN COALESCE(ar.warning_events_30d, 0) > 0 THEN 'Active with warnings'
    ...
    CASE
        ...
        WHEN dps.last_seen_at < NOW() - INTERVAL '35 days' AND COALESCE(ar.warning_events_30d, 0) > 0 THEN 'Review recent OS patch warnings (Drilldown)'
        ...
        WHEN COALESCE(ar.warning_events_30d, 0) > 0 THEN 'Review recent OS patch warnings (Drilldown)'
