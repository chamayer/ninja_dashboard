-- Simplify human-facing device workflow states.
--
-- State is now workflow-only:
--   Fix now, Review, Stale, Ignored, Good
--
-- Details such as degraded agent or cross-customer conflict stay in the
-- Issue text instead of becoming separate state/filter values.
...
classified AS (
    SELECT
        ...
        CASE
            ...
            WHEN b.cross_client_conflict THEN 'Review'
...
SELECT
    ...
    CASE
        ...
        WHEN cross_client_conflict THEN 'Review'
    ...
    CASE
        ...
        WHEN cross_client_conflict THEN 'Device appears under more than one customer'
